...
1. `tg-archive --check-updates` (checks archived messages against Telegram in batches, flagging deleted messages and recording edits in `data.sqlite`; combine with `--recent-days 7` to check only the last N days. `--check-deleted` is supported as an alias).
1. `tg-archive --listen` (listens in real time for live Telegram events such as message deletions, edits, and new messages; can be combined with `--sync`).
